class NavigationLink < ActiveRecord::Base
  ...
  belongs_to :parent_link, class_name: "NavigationLink", foreign_key: :parent_link_id
  has_many :child_links, class_name: "NavigationLink"
  belongs_to :page

  scope :parent_links, where("parent_link_id IS NULL AND page_id IS NULL AND (location IS NULL OR LOCATION = '')")

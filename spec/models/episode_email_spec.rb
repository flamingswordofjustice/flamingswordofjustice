...
describe EpisodeEmail do
  ...
  context "html" do
    ...
    let(:email)   { EpisodeEmail.new(renderer: renderer, stub_mailer: FakeMailer.new) }
  ...
  context "send!" do
    ...
    let(:blacklisted) { EpisodeEmail::RECIPIENT_BLACKLIST.first }
    ...
    context "proofing" do
      context "fails if no user given" do
        subject { EpisodeEmail.new(proofed: true, stub_mailer: mailer, renderer: renderer) }
      ...
      context "is successful if user given" do
        ...
        subject { EpisodeEmail.new(proofed: true, admin_user: stub_model(User), stub_mailer: mailer, renderer: renderer) }
    ...
    context "blacklisting" do
      context "prevents sending in development mode" do
        ...
        subject { EpisodeEmail.new(recipient: blacklisted) }
    ...
    context "when successful" do
      ...
      let(:email)    { EpisodeEmail.new(stub_mailer: mailer, renderer: renderer, recipient: innocent, sender: "show@example.com") }
